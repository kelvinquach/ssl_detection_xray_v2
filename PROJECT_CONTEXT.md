...
#### Resolution update after Phase 2D

```text
The DICOM-to-JPG image representation protocol was subsequently
locked and GPT-reviewed in Phase 2D.1A on 2026-07-15.

Resolved in Phase 2D.1A:
- DICOM decoding policy;
- Modality LUT / RescaleSlope / RescaleIntercept policy;
- VOI LUT / windowing policy;
- MONOCHROME1 and presentation polarity policy;
- intensity clipping and uint8 conversion policy;
- one-channel JPG storage and downstream three-channel replication;
- no-resize / no-crop / no-rotation policy;
...
```text
Image representation protocol: PASS
Required policy coverage: 20/20
Locked dataset counts: PASS
Single-source protocol generation: PASS
Cross-output drift count: 0
Strict YAML loading: PASS
JSON parsing: PASS
Atomic output promotion: PASS
Guardrail unit tests: 31/31 PASS
Forbidden actions avoided: PASS
GPT review: PASS
```
...
```text
Protocol version: 1.0.0
Phase status before GPT review: OPEN_REVIEW_REQUIRED
Locked images: 4,894
...
JPG is accepted as the planned processed training representation,
but no JPG training dataset has yet been created.

coco_master.json remains the official annotation master.

coco_master_jpg.json will be a training derivative that changes only
the image file_name representation.

The DICOM-to-JPG transformation order is locked:
decode → padding mask → modality transform → VOI/windowing →
presentation polarity → uint8 conversion → JPEG encoding.
...
MONOCHROME1 / presentation polarity must be normalized to
MONOCHROME2-equivalent output.

JPG storage is one-channel grayscale mode L.

MMDetection three-channel input will be created by grayscale replication
and validated later in Phase 2D.1C.

No resize, crop, rotation, flip, or transpose is allowed.

BBox scaling is not expected, but bbox invariance remains unvalidated
until Phase 2D.1B-Pilot.

JPEG quality 95 and 100 are the only pilot candidates.

Final JPEG quality is not selected before pilot evidence.
```

Issues / risks:

```text
No DICOM pixel array has yet been decoded under this protocol.

Observed DICOM metadata strata have not yet been enumerated by the pilot.

No JPG fidelity metrics have yet been calculated.

No visual audit has yet been performed.

Final JPEG quality remains unresolved.

BBox geometry preservation has been decided but not empirically validated.

The full JPG dataset and coco_master_jpg.json do not exist.

MMDetection and empty-GT retention have not been validated.
```
...
Phase 2D.1B-Full remains locked until:
