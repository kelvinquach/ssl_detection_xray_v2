...
CONFIGS = {
    "R50": {
        "100pct": "configs/supervised/s3_10_faster_rcnn_r50_fpn_sup_100pct.py",
    },
    "SWIN_T": {
        "100pct": "configs/supervised/s3_10_faster_rcnn_swin_t_fpn_sup_100pct.py",
    },
}

EXPECTED_ANN_FILES = {
    "100pct": "instances_train.json",
}
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="/workspace/ssod/artifacts/preflight/sup/schedule_100pct_10284_preflight.json",
    )
    ...
    for arch, budget_configs in CONFIGS.items():
        ...
        for budget, rel_path in budget_configs.items():
            ...
            config_checks = {
                ...
                "max_iters_is_10284": cfg.train_cfg.max_iters == EXPECTED_UPDATES,
                "target_updates_is_10284": (
                    target_hook.target_updates == EXPECTED_UPDATES
                ),
                "milestones_are_6856_9427": (
                    list(scheduler.milestones) == EXPECTED_MILESTONES
                ),
                "scheduler_end_is_10285": scheduler.end == EXPECTED_END,
    ...
    checks["r50_swint_share_same_100pct_schedule_structure"] = all(
    ...
    report = {
        "schema_version": "1.0",
        "stage": "S3.10",
        "scope": "supervised_100pct_10284_actual_update_scheduler_preflight",
    ...
    if failed_checks:
        print("S3_10_100PCT_10284_SCHEDULE=FAIL")
        return 1

    print("S3_10_100PCT_10284_SCHEDULE=PASS")
    return 0
