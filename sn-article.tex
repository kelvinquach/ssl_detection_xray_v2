...
Đối với lịch huấn luyện 2064 optimizer updates, các mốc giảm learning rate được
đặt xấp xỉ tại update 1376 và 1892. Với ngân sách \(1\%\), các vị trí tương ứng
được điều chỉnh theo tỷ lệ để giữ cấu trúc scheduler tương thích với tổng ngân
sách huấn luyện.
...
Một prediction được ghép với ground truth khi đồng thời thỏa ba điều kiện:
cùng lớp, chưa được ghép với một prediction khác và có
...
Matching được thực hiện theo nguyên tắc category-aware và one-to-one. Từ số
true positive \(TP\) và false negative \(FN\), Recall tại operating point
được tính bởi
...
Các phép so sánh này được điều chỉnh đa kiểm định theo phương pháp Holm như
trình bày ở
Mục~\ref{subsubsec:statistical_multiplicity}.
...
Sáu phép so sánh cặp giữa bốn ngân sách nhãn của RQ3 tạo thành một family riêng
và cũng được điều chỉnh bằng phương pháp Holm. Một khác biệt cụ thể giữa hai
ngân sách chỉ được diễn giải như một kết quả chính thức của RQ3 khi cả omnibus
analysis tương ứng và phép so sánh cặp sau điều chỉnh đều đáp ứng tiêu chí đã
xác định.
